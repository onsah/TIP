...
trait TipSublanguages extends DepthFirstAstVisitor[Unit] {
  ...
  def LanguageRestrictionViolation(message: String, loc: Loc): Nothing =
    throw new TipProgramException(
      s"The analysis requires the program to be in the ${this.getClass} sub-language\n   $message ${loc.toStringLong}")
}

/**
  * In this sub-language, function identifiers can only be used in direct calls, and indirect calls are prohibited.
  */
class NoFunctionPointers(implicit declData: DeclarationData)
    extends TipSublanguages {
    ...
    ast match {
      case ACallFuncExpr(targetFun: AIdentifier, args, _) =>
        targetFun.declaration match {
          ...
          case _ =>
            LanguageRestrictionViolation(
              s"Indirect call not allowed, $targetFun is not a function",
              ast.loc)
        }
      ...
      case id: AIdentifier =>
        id.declaration match {
          case _: AFunDeclaration =>
            LanguageRestrictionViolation(
              s"Identifier $id is a function identifier not appearing in a direct call expression",
              ast.loc)
          case _ =>
        }
...
object NormalizedForPointsToAnalysis extends TipSublanguages {
  ...
  def visit(ast: AstNode, x: Unit): Unit = {
    ast match {
      case AAssignStmt(_: AIdentifier,
                       AAlloc(ANull(_) | ANumber(_, _), _),
                       _)                                 =>
      case AAssignStmt(_: AIdentifier, AVarRef(_, _), _)  =>
      case AAssignStmt(_: AIdentifier, _: AIdentifier, _) =>
      case AAssignStmt(_: AIdentifier,
                       AUnaryOp(DerefOp, _: AIdentifier, _),
                       _)                                                 =>
  ...
  def visit(ast: AstNode, x: Unit): Unit = {
    ast match {
      case AUnaryOp(_: DerefOp.type, _, _) =>
        LanguageRestrictionViolation(s"Pointer load operation is not allowed",
                                     ast.loc)
      case AVarRef(_, _) =>
        LanguageRestrictionViolation(
          s"Pointer reference operation is not allowed",
          ast.loc)
      case as: AAssignStmt =>
        as.left match {
          case _: ADerefWrite =>
            LanguageRestrictionViolation(
              s"Pointer store operation is not allowed",
              ast.loc)
          case _: AIndirectFieldWrite =>
            LanguageRestrictionViolation(
              s"Pointer field store operation is not allowed",
              ast.loc)
          case _ =>
        }
      case _ =>
...
/**
  ...
  */
class NormalizedCalls(implicit declData: DeclarationData)
    extends TipSublanguages {

  def visit(ast: AstNode, x: Unit): Unit =
    ast match {
      case AAssignStmt(_: AIdentifier,
                       ACallFuncExpr(targetFun: AIdentifier, args, _),
                       _) =>
        targetFun.declaration match {
          case _: AFunDeclaration =>
          case _ =>
        ...
        if (args.exists(!_.isInstanceOf[AAtomicExpr]))
          LanguageRestrictionViolation(
            s"One of the arguments $args is not atomic",
            ast.loc)
      ...
      case call: ACallFuncExpr =>
        LanguageRestrictionViolation(
          s"Call $call outside an assignment is not allowed",
          ast.loc)
      case _ => visitChildren(ast, x)
    }
}
      ...
      case as: AAssignStmt =>
        as.left match {
          case _: ADirectFieldWrite =>
            LanguageRestrictionViolation(
              s"Field write operation is not allowed",
              ast.loc)
          case _: AIndirectFieldWrite =>
            LanguageRestrictionViolation(
              s"Pointer field store operation is not allowed",
