...
/**
  ...
  */
class EdgeEnvLattice[D, L <: Lattice, EFL <: EdgeFunctionLattice[L]](
    val edgelattice: EFL)
    extends Lattice {
  ...
  def lub(x: Element, y: Element): Element =
    y.foldLeft(x) {
      case (tm, (dd, ye)) =>
        tm + (dd -> edgelattice.lub(tm.getOrElse(dd, edgelattice.bottom), ye))
    }
  ...
  /**
    ...
    */
  def apply(f: DL => Map[DL, edgelattice.Element],
            x: StateLatticeElement): StateLatticeElement = {
  ...
  def compose2(f: Element, g: DL => Map[DL, edgelattice.Element]): Element =
    build(f.toList.flatMap {
      case ((d1, d2), fe) =>
        g(d2).map { case (d3, ge) => ((d1, d3), ge.composeWith(fe)) }
    })
  ...
  private def build(q: List[((DL, DL), edgelattice.Element)]): Element =
    q.foldLeft(Map[(DL, DL), edgelattice.Element]()) {
      case (tm, (dd, e)) =>
        tm + (dd -> edgelattice.lub(tm.getOrElse(dd, edgelattice.bottom), e))
    }
  ...
  private def build2(q: List[(D, valuelattice.Element)]): StateLatticeElement =
    q.foldLeft(Map[D, valuelattice.Element]()) {
        case (tm, (d, v)) =>
          tm + (d -> valuelattice.lub(tm.getOrElse(d, valuelattice.bottom), v))
      }
  ...
  private def curry[A, B, C](m: Map[(A, B), C]): Map[A, Map[B, C]] =
    m.foldLeft(Map[A, Map[B, C]]()) {
        case (t, ((a, b), c)) => t + (a -> (t.getOrElse(a, Map()) + (b -> c)))
      }
